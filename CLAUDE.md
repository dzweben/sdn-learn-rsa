...
## This repo is the settled pipeline only

`Learn-CR-Pipeline/` contains the production pipeline, its documentation, the results report, and the two
presentations. Nothing else. Exploratory methods, literature, proposals, prior attempts, and old walkthrough
drafts live in the sibling folder `Y1-background-Learn/` and are deliberately kept out of this repo and off the
clusters.
...
| 02 | `02_make_timing.sh` | `events_fixed/` to `timing/` (correct pre-enrichment timing) |
...
The interactive report is `analysis/new-v2/index.html`. The methods walkthrough is
...
Derivatives on the clusters hold only the canonical `feedback_runwise_glm` output per subject
...
3. Exploratory or non-canonical work does not go in this repo or on the clusters. It goes to
   `Y1-background-Learn/`.
4. Safe execution order: fix events, generate timing, run GLM, then the analyses.
5. Documentation is terse and explanatory. No em dashes.
