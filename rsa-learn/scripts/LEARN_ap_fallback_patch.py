...
# Usage: LEARN_ap_fallback_patch.py <ap_tmp> <subj> <runs...>

if len(sys.argv) < 4:
    raise SystemExit("Usage: LEARN_ap_fallback_patch.py <ap_tmp> <subj> <runs...>")
...
def replace_stim_times(lines_in, stim_times_lines):
    ...
    while i < len(lines_in):
        line = lines_in[i]
        ls = line.lstrip()
        if ls.startswith("-regress_stim_times") and "offset" not in ls:
            out.append(line)
            i += 1
...
def build_dsets():
    out = []
    for r in runs:
        out.append(f"\t\t\t{subj_dir}/func/sub-{subj}_task-learn_run-{r}_space-MNI152NLin2009cAsym_desc-preproc_bold.nii.gz \\")
    return out
...
# Ensure -regress_stim_times exists (avoid offset-only match)
if not any(l.lstrip().startswith("-regress_stim_times") and "offset" not in l for l in lines):
    for i, l in enumerate(lines):
        if "-regress_stim_times_offset" in l:
